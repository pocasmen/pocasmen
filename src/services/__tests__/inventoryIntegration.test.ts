...
async function runIntegrationTest() {
    ...
    try {
        console.log('--- INICIANDO TESTE DE INTEGRAÇÃO DE INVENTÁRIO ---');
        await client.query('BEGIN');

        // 1. Inserção de 3 peças simples novas
        console.log('1. Criando 3 peças simples...');
        ...
        console.log('2. Criando peça composta (KIT)...');
        ...
        console.log(`Initial Virtual Stock: ${kit.rows[0].virtual_stock} (Esperado: 0)`);

        // 3. Modificação directa de stock (Físico)
        console.log('3. Adicionando stock físico (+10 em cada)...');
        ...
        console.log(`Virtual Stock depois de +10 físico: ${kit.rows[0].virtual_stock} (Esperado: 10)`);

        // 4. Encomenda de peças (Ordered Quantity)
        console.log('4. Encomendando peças (+5 da Jante)...');
        ...
        kit = await client.query(`SELECT virtual_stock FROM parts WHERE id = $1`, [idKit]);
        console.log(`Virtual Stock (Físico) após encomenda Jante: ${kit.rows[0].virtual_stock} (Esperado: 10 - apenas físico conta para virtual_stock básico)`);

        // 5. Chegada de peças (Arrival: ordered -> stock)
        console.log('5. Chegada de encomenda (5 Jantes chegam)...');
        ...
        console.log(`Part B (Jante) - Stock: ${partB.rows[0].stock_quantity}, Encomendada: ${partB.rows[0].ordered_quantity}`);

        kit = await client.query(`SELECT virtual_stock FROM parts WHERE id = $1`, [idKit]);
        console.log(`Virtual Stock após chegada: ${kit.rows[0].virtual_stock} (Esperado: 10 - porque Pneu e Válvula limitam a 10)`);

        // 6. Teste de limite (Aumentar as outras duas para ver o Kit subir)
        console.log('6. Aumentando peças limitantes (Pneu e Válvula +10)...');
        await inventoryService.updatePartStock(client, idA, { quantity: 10, fromOrder: false, targetStock: StockType.GENERAL });
        await inventoryService.updatePartStock(client, idC, { quantity: 10, fromOrder: false, targetStock: StockType.GENERAL });

        kit = await client.query(`SELECT virtual_stock FROM parts WHERE id = $1`, [idKit]);
        console.log(`Virtual Stock Final: ${kit.rows[0].virtual_stock} (Esperado: 15 - limitado pela Jante que tem 15)`);

        await client.query('ROLLBACK');
        console.log('--- TESTE CONCLUÍDO COM SUCESSO E ROLLBACK EXECUTADO ---');
    } catch (err) {
        await client.query('ROLLBACK');
